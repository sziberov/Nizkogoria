...
window.Translator = class Translator {
	...
	static rules = {
		...
		//	'ьи':	'ьйи',
			...
			'([бклмрф])\\1':																'$1',
			'([бвп])ь(?=[еёиюя])':															'$1ъ',
			'([бвгґджзклмнпрстфхцчш])ь(?=[еёиюя])':											['<<short_u_bypass>>$1', 'softsign-consonant'],
			'в(?=ь?[ \\t\\p{P}]+[кпстфхцчшщ])':												'ф',
			'(?<=^|[\\s\\d\\p{P}]+)[вф](?=[ \\t\\p{P}]+[^аеёиоуыэюя])':						['ў', 'v_l_f-short_u'],
			'(?<=^|[\\s\\d\\p{P}]+|[аеёиоуыэюя])[влф]([ъь](?=[еёиюя])|(?![аеёиоуъыьэюя]))':	['ў', 'v_l_f-short_u'],
			'<<short_u_bypass>>(.)':														['$1$1', 'softsign-consonant'],
			'вот(?=[- \\t]+так)':															'о',
			'(г|ґ)енез':																	'$1енэз',
			'[дт]ь?(?=(д|т)[еёиьюя])':														'$1',
		//	'(?<=\\S{4,})евае':																'иё',
	...
	//	'эта', 'этим', 'этими', 'этих', 'это', 'этого', 'этой', 'этом', 'этому', 'этот', 'этою', 'эту',
		...
		'куда', 'откуда', 'откудова',
		'туда', 'оттуда', 'оттудова',
		'сюда', 'отсюда', 'отсюдова',
		'как', 'так', 'сяк',
